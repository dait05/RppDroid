...
//@ policy by benjamin
import swl.monitor.PolicyManager;
//# end by benjamin
...
public class MonitorManagerService extends IMonitorManager.Stub {
	...
	private boolean fromSetZone = false;
...
*/
	...
	public int updateKernel() 
	{
		...
		if(desc < 0)
		{
			Log.e(TAG, "Device file cannot be opened");
			return -1;
		}
		...
		Log.e(TAG, "External storage path = " + external_storage_path);    
		
		PolicyManager.getDefault().PolicyExternalStorage(desc, external_storage_path );
		
		//PolicyManager.getDefault().PolicyDump(desc);

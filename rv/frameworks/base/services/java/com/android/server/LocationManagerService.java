...
public class LocationManagerService extends ILocationManager.Stub implements Runnable {
	...
	//@ xiaolei use one boolean value to determine whether this receiver is allowed to receive changed location
	private boolean adchoice = true;
	void setAdchoice(boolean ad)
	{
		adchoice = ad;
	} 
	//# xiaolei
	    ...
	    //monitor by xiaolei
	    //if adchoice is false, we reset the location to return the receiver one fake location
	    if(!adchoice)
	    {
		//Location l = new Location(location.getProvider());
		//location.reset();
		//location.setProvider(location.getProvider());
    ...
    public void requestLocationUpdates(String provider, Criteria criteria,
	    ...
	    if(mm.Hook(mm.getPackageNameForMonitoredProcess(getCallingPid()), "LocationManagerService.requestLocationUpdates", mm.Args(provider,minTime,minDistance,listener) )!=0)
	    {
		adchoice = false;
	    }
	    //end by xiaolei
	...
	//monitor by xiaolei
	MonitorManager mm = MonitorManager.getDefault();
	if(mm.Hook(mm.getPackageNameForMonitoredProcess(getCallingPid()),"LocationManagerService.getLastKnownLocation", mm.Args(provider) )!=0)
	{

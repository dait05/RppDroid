...
public class ActivityStack {
	...
	//@xiaolei constrain IPC in same zone
	/*if(!(callingUid==0 || callingUid==1000))
	{
		MonitorManager mm = MonitorManager.getDefault();
		String CallerPname = mm.getPackageNameForMonitoredProcess(callingPid);
		Slog.w(TAG, "CallerPname-->"+CallerPname);
		if(CallerPname!=null && !CallerPname.equals("system_server"))
		{
			String CalleeZname = mm.getZoneFromPackage(aInfo.packageName);
			Slog.w(TAG, "CalleeZname-->"+CalleeZname+" aInfo.packageName-->"+aInfo.applicationInfo.packageName);
			if(CalleeZname!=null)
			{
				
				String CallerZname = mm.getZoneFromPackage(CallerPname);
				Slog.w(TAG, "CalleeZname-->"+CalleeZname + " CallerZname-->"+CallerZname);
				if(CallerZname==null || !CallerZname.equals(CalleeZname))
				{
            				String msg = "Cross Zone Denial: starting " + intent.toString()
                    				+ " from " + callerApp + " (zname=" + CallerZname + ")"
                    				+ " to " + aInfo.packageName + " (zname=" + CalleeZname + ")";
            				Slog.w(TAG, msg);
    ...
    final int startActivityMayWait(IApplicationThread caller,
	    ...
	    int monitorPid = -1;
	    if(caller==null)
		monitorPid = Binder.getCallingPid();
	    else
	    {
		ProcessRecord callerApp = mService.getRecordForAppLocked(caller);
		if(callerApp != null)
			monitorPid = callerApp.pid;
		else
			return START_PERMISSION_DENIED;
	    }
		
	    if(action!=null && (action.equals(Intent.ACTION_CALL) || action.equals(Intent.ACTION_DIAL)))
	    {
		MonitorManager mm = MonitorManager.getDefault();
		if(mm.Hook(mm.getPackageNameForMonitoredProcess(monitorPid), "ActivityStack.startActivityMayWait", mm.Args() )!=0)
		{
			throw new IllegalArgumentException("Phone Call not allowed");
		}
	    }
	    //#xiaolei
	    ...
	    //@xiaolei: profile isolation
	    Log.v(TAG, "Profile Isolation: " + monitorPid + " starts " + aInfo.packageName + " with action " + action);
	    MonitorManager mm = MonitorManager.getDefault();
	    String callerProfile = mm.getZoneFromPackage(mm.getPackageNameForMonitoredProcess(monitorPid));
	    if(callerProfile == null)
		callerProfile = "UnZone";
					//Log.e(TAG, "SMS receiver ResolveInfo" + ((ResolveInfo)o).activityInfo.packageName );//ai.targetActivity);
	    String calleeProfile = mm.getZoneFromPackage(aInfo.packageName);
	    if(calleeProfile == null)
		calleeProfile = "UnZone";
	    //Log.v(TAG, "Profile Isolation: callerProfile-->" + callerProfile + " calleeProfile-->"+calleeProfile);
	    if(callerProfile.equals("UnZone") || calleeProfile.equals("UnZone") || callerProfile.equals(calleeProfile))
	    {
	    }else{
		Log.e(TAG, "Profile Isolation: callerProfile-->" + callerProfile + " calleeProfile-->"+calleeProfile);
	    	throw new IllegalArgumentException("Cross-Profile start activity not allowed");
	    }
	    //add mDelegationSource for ResolverActivity
	    //Log.v(TAG, "intent.getComponent()-->"+intent.getComponent());
	    //Log.v(TAG, "intent.getComponent().getPackageName()-->"+intent.getComponent().getPackageName());
	    //Log.v(TAG, "intent.getComponent().getClassName()-->"+intent.getComponent().getClassName());
	    //if(intent.getComponent().equals("android/com.android.internal.app.ResolverActivity"))
	    if(intent.getComponent().getPackageName().equals("android") && 
		(intent.getComponent().getClassName().equals("com.android.internal.app.ResolverActivity")
		|| intent.getComponent().getClassName().equals("com.android.internal.app.ChooserActivity")))
	    {
		//Log.v(TAG, "intent.getComponent()-->"+intent.getComponent());
		intent.setDelegationSource(callerProfile);
	    }
	    //#xiaolei

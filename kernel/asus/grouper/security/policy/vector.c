...
extern void vector_init(vector *v, size_t elem_size, size_t init_size)
{
	v->elem_size = elem_size;
	v->num_alloc_elems = (int) init_size > 0 ? init_size : VECTOR_INIT_SIZE;
	v->num_elems = 0;
	v->elems = kmalloc(elem_size * v->num_alloc_elems,GFP_KERNEL);
	if (v->elems == NULL)
		return;
	v->free_func = NULL;
}
...
extern void vector_copy(vector *v1, vector *v2)
{
	...
	v2->elems = krealloc(v2->elems, v2->num_alloc_elems * v2->elem_size,GFP_KERNEL);
	if (v2->elems == NULL)
		return;
...
static void vector_grow(vector *v, size_t size)
{
	...
	v->elems = krealloc(v->elems, v->elem_size * v->num_alloc_elems,GFP_KERNEL);
	if (v->elems == NULL)
		return;
}
...
{
	v->num_elems = 0;
	v->elems = krealloc(v->elems, v->num_alloc_elems,GFP_KERNEL);
	if (v->elems == NULL)
		return;
}

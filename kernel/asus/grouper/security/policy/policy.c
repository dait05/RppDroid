...
//#define SYS_CALL_TABLE_ADDRESS 0xc000f304; //new goldfish
//use system call table to make system call in LKM
//#define SYS_CALL_TABLE_ADDRESS 0xc0010f44; //new grouper daiting
#define SYS_CALL_TABLE_ADDRESS 0xc000ed84; //prebuilt mka bootimage daiting
...
//		 struct file *file,	/* ditto */
//		 unsigned int ioctl_num,	/* number and param for ioctl */
//		 unsigned long ioctl_param)//important by daiting for grouper
{
...
struct file_operations Fops = {
	...
	.read = device_read,
	.write = device_write,
	//.ioctl = device_ioctl, //old 3.1 version important
...
//const char* str = "/storage/sdcard/fileprivate/test.txt";
printk(KERN_INFO "daiting---> %d  %s write file xxxxxxxxxx %s and  mode %d. \n", pid, p1->pname, filename, mode);
//if (strstr(filename, "storage/sdcard")) {
/*if (strstr(filename, "storage/emulated/legacy")) {
...
printk(KERN_INFO "daiting 1 %s. \n", tempstr);
		rm_substr(tempstr, "/storage/emulated/legacy");//asumptions on the directory
printk(KERN_INFO "daiting 2 %s. \n", tempstr);

	char temp_filename[612];
	const char* str = "/storage/emulated/legacy/Android/data/";
...
//printk(KERN_INFO "daiting---> %d %s mkdir yyyyyyyyyyyyyyyyyyyyyy %s. \n", pid, p1->pname, filename);

/* important for grouper  daiting

<6>[  919.184889] daiting---> 1499 com.example.filetest mkdir yyyyyyyyyyyyyyyyyyyyyy /storage/emulated/0/fileprivate. 
<6>[  919.185623] daiting---> 149 /system/bin/sdcard mkdir yyyyyyyyyyyyyyyyyyyyyy /data/media/0/fileprivate. 

*/


//const char* str = "/storage/sdcard/fileprivate";
if (strstr(filename, "storage/emulated/0")) {
	printk(KERN_INFO "daiting---> %d mkdir hit %s. \n", pid, filename);
if(!(p1->pname))
	return original_sys_mkdir(filename, mode);
	...
	const char* str = "/storage/emulated/0/Android/data/";
	//const char* str2 = "/test.txt";
	strncpy(temp_filename, str, strlen(str));
	strncpy(temp_filename+strlen(str), p->pname, strlen(p->pname));
...
printk(KERN_INFO "daiting  %d mkdir file name %s. \n", pid, temp_filename);
		...
		tempstr[strlen(filename)] = '\0';	
printk(KERN_INFO "daiting 1 %s. \n", tempstr);
		rm_substr(tempstr, "/storage/emulated/0");//asumptions on the directory
printk(KERN_INFO "daiting 2 %s. \n", tempstr);

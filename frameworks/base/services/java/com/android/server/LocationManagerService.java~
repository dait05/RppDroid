...
//added by daiting
//System.out.println("Hooked by daiting loc: " + mContext.); context is system server
...
//added by daiting
private int mygetCallerAllowedResolutionLevel(String pname, String magic) {

	//file search logic
	File folder = new File("/data/data/mytest/");
...
try{
	for (File file : list) {
		if(file.isFile() && file.getName().equals(pname)) {
			System.out.println("daiting file match: "+pname);
			BufferedReader in = new BufferedReader(new FileReader(file));
			String line = in.readLine();
			String line2 = in.readLine();
			in.close();
			System.out.println("daiting file line: "+line);
			System.out.println("daiting file line2: "+line2);
			PackageManager pm = mContext.getPackageManager();
			ApplicationInfo myinfo = pm.getApplicationInfo(line, 128);
			System.out.println("daiting file uid1: "+myinfo.uid);
			System.out.println("daiting file uid2: "+Binder.getCallingUid());
			System.out.println("daiting file pid: "+Binder.getCallingPid());
		
			if(line.equals("com.cyanogenmod.trebuchet")) {
				p = Binder.getCallingPid();
				u = Binder.getCallingUid();
			} else {
				p = Integer.parseInt(line2);
				u = myinfo.uid;		
			}
			break;
		}
...
System.err.println("daiting p u1: "+p+" "+u);
System.err.println("daiting server magic: "+magic);
...
if (magic.contains(":")) {
	...
	try{
		...
		ApplicationInfo myinfo1 = pm1.getApplicationInfo(part1, 128);
			if(part1.equals("com.cyanogenmod.trebuchet")) {
				p = Binder.getCallingPid();
				u = Binder.getCallingUid();
			} else {
	...
	}catch (Exception e){System.err.println("daiting p u exception");}

} else {
throw new IllegalArgumentException("daiting magic=" + magic);
}

System.err.println("daiting p u2: "+p+" "+u);
	//return
        return getAllowedResolutionLevel(p, u);
    }
//
...
//modified by daiting
    @Override
    public Location getLastLocation(LocationRequest request, String packageName, String magic) {
        if (D) Log.d(TAG, "getLastLocation: " + request);
//added by daiting
//System.out.println("Hooked daiting location: "+packageName);//packagename is the direct client caller,eg. gpstest

        if (request == null) request = DEFAULT_LOCATION_REQUEST;
//added by daiting
	int allowedResolutionLevel = mygetCallerAllowedResolutionLevel(packageName, magic);
        //int allowedResolutionLevel = getCallerAllowedResolutionLevel();
System.err.println("daiting location0");
        checkPackageName(packageName);
        //deleted by daiting for rv, this will usually block the usage of the location manager
...
System.err.println("daiting location01");
        try {
...
System.err.println("daiting location1");
            ...
            synchronized (mLock) {
                ...
                LocationProviderInterface provider = mProvidersByName.get(name);
System.err.println("daiting location11");
                if (provider == null) return null;
System.err.println("daiting location12");
                if (!isAllowedByUserSettingsLocked(name, uid)) return null;
System.err.println("daiting location13");
                Location location;
                if (allowedResolutionLevel < RESOLUTION_LEVEL_FINE) {
...
System.err.println("daiting location2");
                    location = mLastLocationCoarseInterval.get(name);
                } else {
System.err.println("daiting location3");
                    location = mLastLocation.get(name);
                }
                if (location == null) {
System.err.println("daiting location4");
                    return null;
                }
                if (allowedResolutionLevel < RESOLUTION_LEVEL_FINE) {
                    Location noGPSLocation = location.getExtraLocation(Location.EXTRA_NO_GPS_LOCATION);
                    if (noGPSLocation != null) {
System.err.println("daiting location5");
                        return new Location(mLocationFudger.getOrCreate(noGPSLocation));
                    }
                } else {
System.err.println("daiting location6");

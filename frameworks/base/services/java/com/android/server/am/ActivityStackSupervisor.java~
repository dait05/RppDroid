...
//@ monitor by daiting
import android.monitor.MonitorManager;
...
if (componentSpecified) {
	ComponentName mycn = intent.getComponent();//callee
	System.out.println("daiting callee act: " + mycn.getPackageName() + ":" + mycn.getClassName());
}
...
//system_server is the binder
System.out.println("daiting callee Ainfo name: " + aInfo.name);//callee activity ainfo from callee
...
//added by daiting
//System.out.println("Hooked by daiting rsal1: " + r.intent.mygetPname());
...
//added by daiting
//System.out.println("Hooked by daiting ssal: " + r.intent.mygetPname());
...
//added by daiting important
System.out.println("daiting sal caller actname: "+sourceRecord.info.name);//caller activity name
System.out.println("daiting sal caller pid: "+sourceRecord.app.pid);//caller activity in pid
System.out.println("daiting sal caller old pname: "+sourceRecord.intent.mygetPname());//caller intent string

//file
File file = new File("/data/data/mytest/"+(String)aInfo.packageName);
try{
	FileOutputStream f = new FileOutputStream(file);
	PrintWriter pw = new PrintWriter(f);
	pw.println(callingPackage);
	pw.println(sourceRecord.app.pid);
	pw.flush();
	pw.close();
	f.close();
}catch (IOException e) {
...
//history based
	if(sourceRecord.intent.mygetPname().equals("notset"))	
		intent.mysetPname(callingPackage+":"+sourceRecord.app.pid);//first time
	else {
		intent.mysetPname(sourceRecord.intent.mygetPname()+"+"+callingPackage+":"+sourceRecord.app.pid);//not first time
	}
System.out.println("daiting sal caller new pname: "+intent.mygetPname());//caller info

//write to kernel, added by daiting
MonitorManager mm = MonitorManager.getDefault();
System.out.println("daiting sal mm and setZone mm: "+mm);//caller info
int retval = mm.setZone(aInfo.packageName, intent.mygetPname());
System.out.println("daiting sal mm and setZone retval: "+retval);//caller info
//mm.updateKernel();
//
...
//daiting, new activityrecord for callee
